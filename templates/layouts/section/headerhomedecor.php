<header class="element-header notsticky d-none-m <?= ($source == 'index')?"shadow-sticky ":"box-shadow headersticky"?>"
                    ...
                    <ul class="nav-menu d-flex justify-content-between">
                        ...
                        <li class=" p-relative nav-menu-line">
                            ...
                            <h2>
                                ...
                                <div class="sub-menu">
                                    <ul>
                                        <?php foreach($danhmuc1menu as $key => $value){ 
                                        ...
                                        <li><a
                                            ...
                                            <ul class="menucon">
                                                <?php foreach($c2 as $k => $v){
                                                    $c3 = $db->rawQuery("select id ,tenkhongdau_$lang as tenkhongdau , ten_$lang as ten from #_baiviet_item where hienthi=1 and id_cat=?  order by stt asc,id desc",array($v['id']));
                                                    ?>
                                                <li><a
                                                        href="<?= $value['type']?>/<?= $value['tenkhongdau']?>/<?= $v['tenkhongdau']?>"><?= $v['ten']?></a>
                                                    <ul class="menuchau">
                                                        <?php foreach($c3 as $ki => $va){ ?>
                                                        <li><a href="<?= $value['type']?>/<?= $value['tenkhongdau']?>/<?= $v['tenkhongdau']?>/<?= $va['tenkhongdau']?>"><?= $va['ten']?></a>
                                                        </li>
                                                        <?php } ?>
                                                    </ul>
                                                </li>
                        ...
                        <li class="wrapper-nav__menu-logo t-center items-lastchild">
                            <a href="" class="p-relative d-block">
                                <img src="<?=_upload_hinhanh_l.$logo['photo_'.$lang]?>" alt="Logo trang chủ"
                                    class="responLogo"
                                    style="<?= ($source == 'index')?"":"filter: brightness(1) invert(0) !important;"?>">
                                <div class="star-box">
                        ...
                        <li class="<?php if($com=='bao-gia') echo ' active';?> p-relative nav-menu-line">
                            ...
                            <h2>
                                <?php }?>
                                <a href="bao-gia" title="BÁO GIÁ" rel="dofollow">BÁO GIÁ</a>
