...
_FRAMEWORK = {
	...
	simple:()=>{
		...
		$("img").each(function(){
			...
			if(alt == "" || alt == undefined || alt.length == null){
				$(this).attr("alt",'Việt FURINUTE');
			}
		});
	},
	...
	captcha: ()=>{
		if($("#captchacode").length > 0){
			getCaptcha("#captchacode");
		}
		if($("#captchacodecontact").length > 0 ){
			getCaptcha("#captchacodecontact");
		}
		...
		function getCaptcha(target){
			$.post("ajax/captcha.php",{},function(data,status){
				if(status == "success"){
					$(target).html(data);
				}
			})
	...
	mmenu:function(){
		$('#dangkyname').placeholderTypewriter({text: _HOVATEN});
		if(exists("nav#mmenu")){
		...
		$('#dangkyEmail').placeholderTypewriter({text: _NHAPEMAIL});
		if(exists("nav#mmenu")){
		...
		$('#dangkyPhone').placeholderTypewriter({text: _NHAPSODIENTHOAI});
		if(exists("nav#mmenu")){
		...
		$('#dangkyMessage').placeholderTypewriter({text: _NOIDUNG});
		if(exists("nav#mmenu")){
		...
		$('#dangkyCapcha').placeholderTypewriter({text: _NHAPMACAPCHA});
		if(exists("nav#mmenu")){
		...
		$('#mailfooter').placeholderTypewriter({text: _NHAPEMAILCUABAN});
		if(exists("nav#mmenu")){
	...
	mobiTool: function () {
		...
		$('.toc-wrapper').click(function () {
			if($(this).hasClass('deletebefore')){
				$(this).removeClass('deletebefore');
			}else{
				$(this).addClass('deletebefore');
			}
	...
	dangkyContact: function () {

		$('#dangky-contact').click(function () {

			var _o = $(this);

			var _fn = $('#dangkyname').val();

			var _el = $('#dangkyEmail').val();

			var _pn = $('#dangkyPhone').val();

			var _obj = $('#dangkySubject').val();

			var _capcha = $('#dangkyCapcha').val();
		...
		if (_TOC == 1 || _LIST_TOC == 1) {
			...
			$('a#toc').click(function () {
				...
				if(!_this.hasClass('active')){
					...
					$('.col__left').css({'min-width':'30%','transition':'all 0.3s ease'});
					$('.title__toc').css('display','block');
					...
					$('.col__left').css({'min-width':'0%','transition':'all 0.3s ease'});

					$('.title__toc').css('display','none');
				}
