...
$limitsp=$row_setting['limit_sp'];
    ...
    $tktc1 = $db->rawQuery("select id , ten_$lang as ten , tenkhongdau_$lang as tenkhongdau ,mota_$lang as mota, type ,  photo from #_baiviet_list where noibat=1 and hienthi=1 and type=? order by id desc",array('dich-vu'));

    $pros = $db->rawQuery("select ten_$lang,photo,photo1,tenkhongdau_$lang as tenkhongdau , link , type , id from #_baiviet where hienthi=1 and noibat=1 and type=? order by id desc limit 0,$limitsp ",array('san-pham'));
	
	$tintuc = $db->rawQuery("select ten_$lang as ten , tenkhongdau_$lang as tenkhongdau ,mota_$lang as mota, type , ngaytao, photo , luotxem , id_tacgia from #_baiviet where hienthi=1 and type=? order by id desc",array('tin-tuc'));

	$tktc = $db->rawQuery("select id , ten_$lang as ten , tenkhongdau_$lang as tenkhongdau ,mota_$lang as mota, type ,  photo from #_baiviet where hienthi=1 and type=? order by id desc",array('dich-vu'));
    ...
    $videos=$db->rawQuery("select id, ten_$lang as ten, mota_$lang as mota, links,photo from #_video where noibat=1 and type=? order by stt asc,id desc",array('video'));

    $danhmucNoibat = $db->rawQuery("select id , ten_$lang as ten, tenkhongdau_$lang as tenkhongdau,type from #_baiviet_list where hienthi=1 and noibat=1 and type=?",array('san-pham'));
